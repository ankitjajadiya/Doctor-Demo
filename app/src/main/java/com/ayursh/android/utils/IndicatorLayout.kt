...
class IndicatorLayout : LinearLayout {
    ...
    private fun updateIndicators() {
        ...
        for (i in 0 until indicatorCount) {
            ...
            val layoutParams = LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, px(4f))
            layoutParams.weight=1f
            layoutParams.setMargins(px(3f), px(3f), px(3f), px(3f))
            indicator.layoutParams = layoutParams
            indicator.setBackgroundResource(R.drawable.intro_indicator_unselected)
        ...
        if (position >= 0 && position <= indicatorCount) {
            ...
            for (index in 0 until indicatorCount) {
                ...
                if (index == selectedPosition) {
                    indicator.setBackgroundResource(R.drawable.intro_indicator_selected)
                } else {
                    indicator.setBackgroundResource(R.drawable.intro_indicator_unselected)
                }
            }
